...
export default function App() {
  ...
  const [formFiltros, setFormFiltros] = useState({
    dataInicial: "", dataFinal: "", clienteId: "", valorMin: "", valorMax: "", produtoId: ""
  })
    ...
    async function gerarRelatorio() {
      ...
      try {
        ...
        vendas?.forEach(venda => {
          ...
          const itensFiltrados = formFiltros.produtoId ? itensDaVenda.filter((i: any) => i.produto_id.toString() === formFiltros.produtoId) : itensDaVenda
        ...
        setDadosRelatorio({ faturamento: faturamentoTotal, lucro: lucroTotal, qtdVendas: qtdVendasValidas, ticketMedio, margem, topProdutos: arrayTopProdutos.map(p => ({ ...p, maxQtd })) })
  ...
  const aplicarFiltrosPersonalizados = () => { setFiltroData("personalizado"); setTriggerFiltro(prev => prev + 1) }
  const limparFiltrosPersonalizados = () => { setFormFiltros({ dataInicial: "", dataFinal: "", clienteId: "", valorMin: "", valorMax: "", produtoId: "" }); setFiltroData("hoje") }
  ...
  /* --- LÓGICA DO PDV COM LIMITES DE ESTOQUE --- */
    ...
    return (
      <div className="relative flex flex-col h-[100dvh] bg-slate-100 dark:bg-slate-950 items-center justify-center p-4 font-sans transition-colors">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 w-full max-w-sm text-left">
          <div className="text-center mb-6"><span className="text-4xl">🔐</span><h2 className="text-2xl font-black text-slate-800 dark:text-slate-100 mt-2">PDV Login</h2></div>
      ...
      <main className="flex-1 overflow-y-auto relative">
              ...
              <form onSubmit={salvarProduto} className="space-y-4 text-left">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input placeholder="0,00" inputMode="decimal" value={precoForm} onChange={(e) => setPrecoForm(e.target.value)} className="h-12 bg-slate-50 dark:bg-slate-950 border-orange-200 dark:border-orange-900/50 dark:text-white focus-visible:ring-orange-500" required />
                  </div>
                </div>
        ...
        {abaAtual === 'painel' && userRole === 'gerente' && (
          <div className="p-4 pt-6 h-full bg-slate-50 dark:bg-slate-950 overflow-y-auto pb-24 transition-colors">
            ...
            <button onClick={() => setMostrarMaisFiltros(!mostrarMaisFiltros)} className="w-full mb-6 flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 bg-transparent border-none outline-none py-2 hover:text-orange-500 transition-colors">
            ...
            {mostrarMaisFiltros && (
              <div className="bg-white dark:bg-slate-900 p-4 rounded-xl mb-6 shadow-sm border border-slate-200 dark:border-slate-800 space-y-4 text-left animate-in slide-in-from-top-2 duration-300">
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Data Inicial</label><Input type="date" value={formFiltros.dataInicial} onChange={e => setFormFiltros({...formFiltros, dataInicial: e.target.value})} className="h-10 mt-1 text-sm bg-slate-50 dark:bg-slate-950 dark:text-white dark:border-slate-800" /></div>
                  <div><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Data Final</label><Input type="date" value={formFiltros.dataFinal} onChange={e => setFormFiltros({...formFiltros, dataFinal: e.target.value})} className="h-10 mt-1 text-sm bg-slate-50 dark:bg-slate-950 dark:text-white dark:border-slate-800" /></div>
                </div>

                <div>
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400">Cliente Específico</label>
                  <select className="w-full h-10 mt-1 px-3 text-sm rounded-md border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500" value={formFiltros.clienteId} onChange={e => setFormFiltros({...formFiltros, clienteId: e.target.value})}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Valor Mín. (R$)</label><Input type="number" placeholder="0.00" value={formFiltros.valorMin} onChange={e => setFormFiltros({...formFiltros, valorMin: e.target.value})} className="h-10 mt-1 text-sm bg-slate-50 dark:bg-slate-950 dark:text-white dark:border-slate-800" /></div>
                  <div><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Valor Máx. (R$)</label><Input type="number" placeholder="999.00" value={formFiltros.valorMax} onChange={e => setFormFiltros({...formFiltros, valorMax: e.target.value})} className="h-10 mt-1 text-sm bg-slate-50 dark:bg-slate-950 dark:text-white dark:border-slate-800" /></div>
                </div>
                
                <div className="text-left relative">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400">Produto Específico</label>
                  <Input placeholder="Digite para buscar produto..." value={formFiltros.produtoId ? produtos.find(p => p.id.toString() === formFiltros.produtoId)?.nome || "" : ""} onChange={(e) => { setBusca(e.target.value); if (e.target.value === "") setFormFiltros({...formFiltros, produtoId: ""}); }} onFocus={() => setMostrarResultadosPainel(true)} className="h-10 mt-1 text-sm bg-slate-50 dark:bg-slate-950 dark:text-white dark:border-slate-800" />
                    ...
                    <div className="absolute top-full mt-1 left-0 right-0 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 max-h-44 overflow-y-auto z-50 text-left">
                      {produtosFiltrados.length === 0 ? (
                        ...
                        produtosFiltrados.map(p => (
                          <div key={p.id} onClick={() => { setFormFiltros({...formFiltros, produtoId: p.id.toString()}); setMostrarResultadosPainel(false); setBusca(""); }} className="p-3 border-b dark:border-slate-700 last:border-b-0 active:bg-slate-100 dark:active:bg-slate-700 flex justify-between items-center cursor-pointer transition-colors">
            ...
            {carregandoRelatorio ? (
              <div className="flex flex-col items-center justify-center h-40 text-slate-400"><span className="text-2xl mb-2 animate-spin">⏳</span><p className="text-sm font-medium">Calculando métricas...</p></div>
            ) : (
              <div className="space-y-6 animate-in fade-in duration-300 text-left">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 text-left">
                    <span className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase mb-1">Faturamento</span><span className="text-xl font-black text-slate-800 dark:text-slate-100">R$ {dadosRelatorio.faturamento.toFixed(2)}</span>
                  </div>
                  <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-emerald-200 dark:border-emerald-900/30 text-left">
                    <span className="block text-xs font-bold text-emerald-600 dark:text-emerald-500 uppercase mb-1">Lucro Bruto</span><span className="text-xl font-black text-emerald-600 dark:text-emerald-400">R$ {dadosRelatorio.lucro.toFixed(2)}</span><span className="block text-[10px] text-emerald-500 mt-1 font-semibold">Margem: {dadosRelatorio.margem.toFixed(1)}%</span>
                  </div>
                  <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 text-left">
                    <span className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase mb-1">Ticket Médio</span><span className="text-xl font-black text-slate-800 dark:text-slate-100">R$ {dadosRelatorio.ticketMedio.toFixed(2)}</span>
                  </div>
                  <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 text-left">
                    <span className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase mb-1">Vendas</span><span className="text-xl font-black text-slate-800 dark:text-slate-100">{dadosRelatorio.qtdVendas}</span><span className="block text-[10px] text-slate-400 mt-1 font-semibold">Recibos emitidos</span>
                  </div>
                </div>
                        ...
                        return (
                          <div key={prod.nome} className="relative">
                            <div className="flex justify-between items-end mb-1 relative z-10">
                              <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{index + 1}. {prod.nome}</span><span className="text-xs font-bold text-slate-900 dark:text-slate-100">{prod.quantidade} un</span>
                            </div>
                            <div className="w-full h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-orange-500 rounded-full" style={{ width: `${porcentagemBarra}%` }}></div></div>
