...
export default function CaixaMobile({ userEmail, mostrarAlerta }: CaixaMobileProps) {
  ...
  // 2. Lógica da Câmera (Leitor de Código de Barras)
  useEffect(() => {
    ...
    if (lendoCodigo) {
      setTimeout(() => {
        ...
        html5QrCode.start(
          ...
          () => {
            // Ignora os erros constantes de "código não encontrado no frame"
          }
        ).catch(() => { 
          setLendoCodigo(false); 
  ...
  const alterarQuantidade = (id: number, novaQuantidade: number | string) => {
    if (novaQuantidade === 0 || novaQuantidade === "0") {
      return setCarrinho(prev => prev.filter(item => item.produto.id !== id));
    }
  ...
  const finalizarVenda = async () => {
    ...
    try {
      ...
      if (vendaError || !vendaData || vendaData.length === 0) throw new Error("Erro ao gerar o registro da venda.")
      ...
      console.error(error); 
      mostrarAlerta("Falha na Venda", "Ocorreu um erro inesperado ao tentar registrar esta venda. Verifique sua conexão.", "erro"); 
    } finally { 
  ...
  const clientesFiltrados = clientes.filter(cliente => 
    cliente.nome.toLowerCase().includes(nomeClientePDV.toLowerCase())
  )
  ...
  return (
    <div className="flex flex-col h-full w-full relative text-left" onClick={() => { setMostrarResultados(false); setMostrarClientes(false); }}>
      ...
      <div className="bg-white dark:bg-slate-900 p-4 pt-6 shadow-sm z-20 relative border-b dark:border-slate-800 transition-colors" onClick={(e) => e.stopPropagation()}>
        <div className="relative flex items-center w-full">
          <Input 
            ...
            className="h-12 text-base bg-slate-50 dark:bg-slate-950 dark:text-white dark:border-slate-800 w-full" 
          />
          {/* O botão do scanner saiu daqui! */}
        </div>
              ...
              produtosFiltrados.map(produto => {
                ...
                return (
                  <div 
                    ...
                    className={`p-3 border-b dark:border-slate-700 active:bg-slate-100 dark:active:bg-slate-700 flex justify-between items-center transition-colors ${semEstoque ? 'opacity-50 cursor-not-allowed bg-slate-50 dark:bg-slate-900/50' : 'cursor-pointer'}`}
                  >
      ...
      {/* LISTA DO CARRINHO (Aumentei o pb-64 para a lista não sumir atrás da nova barra maior) */}
            ...
            <div key={item.produto.id} className="bg-white dark:bg-slate-900 p-3 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col gap-3 transition-colors">
              <div className="flex justify-between items-center w-full gap-2">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <button type="button" onClick={() => alterarQuantidade(item.produto.id, 0)} className="flex items-center justify-center h-8 w-8 text-slate-400 hover:text-red-500 bg-transparent border-0 rounded-md transition-colors">
                    <X className="h-5 w-5" />
                  </button>
                </div>
              </div>
      ...
      {/* ÁREA DE FINALIZAR VENDA E LEITOR (BOTTOM BAR) */}
      <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-slate-900 p-4 border-t dark:border-slate-800 shadow-[0_-4px_10px_-5px_rgba(0,0,0,0.1)] z-20 transition-colors" onClick={(e) => e.stopPropagation()}>
        ...
        {/* NOVO BOTÃO DE ESCANEAR FULL-WIDTH */}
        <button 
          type="button" 
          onClick={() => setLendoCodigo(true)} 
          className="w-full h-12 mb-3 flex items-center justify-center gap-2 border-2 border-orange-500 text-orange-600 dark:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 font-bold rounded-xl transition-colors"
        >
        ...
        <div className="mb-3 relative">
          <Input 
            ...
            value={nomeClientePDV} 
            onChange={(e) => { setNomeClientePDV(e.target.value); setMostrarClientes(true) }} 
            onFocus={() => setMostrarClientes(true)} 
            className="h-10 bg-slate-50 dark:bg-slate-950 dark:text-white dark:border-slate-800" 
          />
      ...
      {vendaSucesso && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-all">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-6 w-full max-w-sm flex flex-col items-center text-center animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-800">
            <div className="w-16 h-16 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center mb-4 shadow-inner text-emerald-600">
